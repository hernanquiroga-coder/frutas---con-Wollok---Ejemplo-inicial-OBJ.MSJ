...
// Frutas y otros comestibles 
    ...
    object mandarina {
        ...
        method pasarUnTiempoDeterminado() {tiempo = tiempo + 4}
        method pasarElTiempoDe(_dias) {tiempo = tiempo + _dias}
...
// El personaje de la historia
    ...
    object cachito {
        ...
        method comer(_unaComida) {felicidad = felicidad + _unaComida.azucar()}
        method felicidad() = felicidad
        method darseUnGusto() {self.comer(alimentoPreferido)}
        method comprar() {banco.debitar(alimentoPreferido.precio())}
    }
...
// El banco que tiene nuestro amigo Cachito
    ...
    object banco {
        var saldo = 1000
        method debitar(_monto) {saldo = saldo - _monto}
    }